...
async function main() {
  ...
  const searchPass = searchResponse.status() === 200 && searchJson.total >= 1 && searchUiState.rowCount >= 1 && searchUiState.badges.length >= 1 && searchUiState.firstLinks.some(l => l.text.includes('Mapa') && /^https:\/\/www\.openstreetmap\.org/.test(l.href || ''));
  ...
  await page.click('#results-body tr[data-company-id]');
